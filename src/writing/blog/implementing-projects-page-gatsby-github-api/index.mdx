...
We are using [axios](https://github.com/axios/axios) to make requests to the API. We could alse use the native `fetch` API for this but `axios` makes it very easy to set timeout, cancel requests and set authorization headers if we need to authenticate with an access token. GitHub API requires a token to make 60+ requests per hour.
